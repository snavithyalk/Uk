...
db.ref('requests').on('value', (snapshot) => {
    ...
    if (currentUser) {
        ...
        if (currentUser.role === 'provider') updateProviderPanel();
...
window.renderTrackingDashboard = function() {
    ...
    myJobs.forEach(job => {
        ...
        let locationInfo = job.liveLocation ? `
            <div style="margin-top:1rem; padding:10px; background:rgba(41, 121, 255, 0.1); border-radius:8px; display:flex; align-items:center; gap:10px;">
                <span style="font-size:1.2rem;">📍</span>
                <div>
                    <div style="font-weight:bold; font-size:0.8rem; color:var(--primary);">Live Location Shared</div>
                    <div style="font-size:0.7rem; color:#aaa;">Last update: ${new Date(job.liveLocation.lastUpdate).toLocaleTimeString()}</div>
                    <a href="https://www.google.com/maps?q=${job.liveLocation.lat},${job.liveLocation.lng}" target="_blank" style="color:var(--primary); font-size:0.8rem; text-decoration:none;">View on Map</a>
                </div>
            </div>
        ` : '';
...
window.updateProviderPanel = function () {
    ...
    myJobs.forEach(job => {
        ...
        li.innerHTML = `
            ...
            <div style="display:flex; gap:10px; margin-top:10px; flex-wrap:wrap;">
                ${job.status === 'assigned' ? `<button onclick="updateJobStatus('${job.id}', 'accepted')" class="btn-primary">Accept Job</button>` : ''}
